"""Gemini implementation of the :class:`ChatLLM` interface.

Wraps ``langchain-google-genai``'s ``ChatGoogleGenerativeAI`` (Gemini 2.5 Flash)
and adapts it to the app's narrow LLM capability: text completion, validated
structured output, and token streaming. This is the only module that imports the
Gemini SDK, so the rest of the codebase (and the tests) stay provider-agnostic.
"""
...
class GeminiChatModel:
    """A :class:`ChatLLM` backed by Google Gemini."""
    ...
    async def complete(self, *, system: str, prompt: str) -> LLMResult:
        ...
        logger.info("gemini.complete", model=self.model_name, total_tokens=usage.total_tokens)
